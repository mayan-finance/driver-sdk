...
export class Relayer {
	...
	private async tryProgressFulfill(swap: Swap) {
		...
		let [destState, destEvmOrder, sourceState, sourceEvmOrder] = await Promise.all([
			...
			isEvmSrc ? this.walletHelper.getReadContract(swap.destChain).orders(swap.orderHash) : null,
			isSolSrc ? getSwiftStateSrc(this.solanaConnection, new PublicKey(swap.stateAddr)) : null,
			isEvmDst ? this.walletHelper.getReadContract(swap.sourceChain).orders(swap.orderHash) : null,
		]);
	...
	async relay(swap: Swap) {
		try {
			...
			if (`0x${orderHashV2.toString('hex')}` !== swap.orderHash) {
	...
	private isMayanInitiatedSwap(swap: Swap) {
		...
		if (swap.sourceChain === CHAIN_ID_SOLANA) {
			return swap.mayanAddress === this.contractsConfig.contractsV2[swap.sourceChain];
			...
			return ethers.getAddress(swap.mayanAddress) === this.contractsConfig.contractsV2[swap.sourceChain];
		} else {
