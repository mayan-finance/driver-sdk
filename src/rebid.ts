...
import { BorshInstructionCoder } from 'anchor30';
import { SwiftV2AuctionIdl as SwiftAuction } from './abis/swift-auction-v2.idl';
import { RpcConfig } from './config/rpc';
import logger from './utils/logger';
import { AuctionAddressV2Solana } from './config/contracts';
...
export class ReBidListener {
	...
	public async start() {
		...
		try {
			...
			const auctionProgram = AuctionAddressV2Solana;
				...
				try {
					if (data.transaction?.transaction) {
						...
						for (let ix of message.instructions) {
							...
							// TODO
							return;
							// orderHash = reconstructOrderHashV2(
